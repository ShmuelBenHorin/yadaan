...
class _PS extends State<PaywallSheet>{
  ...
  @override Widget build(BuildContext context){
    ...
    return Container(height:MediaQuery.of(context).size.height*0.85,
      ...
      child:Column(children:[
        Container(margin:const EdgeInsets.only(top:12),width:40,height:4,decoration:BoxDecoration(color:Pal.ts.withOpacity(0.4),borderRadius:BorderRadius.circular(2))),
        Expanded(child:SingleChildScrollView(padding:const EdgeInsets.fromLTRB(24,20,24,0),child:Column(children:[
          Container(width:76,height:76,decoration:BoxDecoration(shape:BoxShape.circle,gradient:const LinearGradient(colors:[Color(0xFFFF9F0A),Color(0xFFFF6B00)]),boxShadow:[BoxShadow(color:Pal.premium.withOpacity(0.6),blurRadius:24)]),child:const Center(child:Text('\u{1F451}',style:TextStyle(fontSize:38)))),
          const SizedBox(height:16),
          const Text('\u05D9\u05D3\u05E2\u05DF \u05E4\u05E8\u05D5',style:TextStyle(color:Pal.tp,fontSize:28,fontWeight:FontWeight.w900)),
          const SizedBox(height:18),
          _bf('\u{1F534}','\u05E9\u05DC\u05D1\u05D9\u05DD \u05E7\u05E9\u05D9\u05DD \u05E4\u05EA\u05D5\u05D7\u05D9\u05DD'),
          _bf('\u26A1','\u05D0\u05E0\u05E8\u05D2\u05D9\u05D4 48 (\u05E4\u05D9 4)'),
          _bf('\u{1F6AB}','\u05DC\u05DC\u05D0 \u05E4\u05E8\u05E1\u05D5\u05DE\u05D5\u05EA'),
          _bf('\u{1F513}','\u05D2\u05D9\u05E9\u05D4 \u05DC\u05DB\u05DC \u05D4\u05EA\u05DB\u05E0\u05D9\u05DD \u05D4\u05E2\u05EA\u05D9\u05D3\u05D9\u05D9\u05DD'),
          const SizedBox(height:24),
          Row(children:_planTiles()),
          const SizedBox(height:16),
          GestureDetector(onTap:()=>setState(()=>_showCode=!_showCode),child:Text('\u05D9\u05E9 \u05DC\u05DA \u05E7\u05D5\u05D3 \u05D2\u05D9\u05E9\u05D4?',style:TextStyle(color:Pal.ts.withOpacity(0.6),fontSize:12,decoration:TextDecoration.underline))),
          if(_showCode)...[const SizedBox(height:12),Row(children:[
            Expanded(child:TextField(controller:_ctrl,style:const TextStyle(color:Pal.tp),decoration:InputDecoration(hintText:'\u05D4\u05D6\u05DF \u05E7\u05D5\u05D3...',hintStyle:const TextStyle(color:Pal.ts),filled:true,fillColor:Pal.card,border:OutlineInputBorder(borderRadius:BorderRadius.circular(12)),contentPadding:const EdgeInsets.symmetric(horizontal:16,vertical:12)))),
            const SizedBox(width:10),
            GestureDetector(onTap:()=>widget.onCode(_ctrl.text.trim()),child:Container(padding:const EdgeInsets.symmetric(horizontal:16,vertical:14),decoration:BoxDecoration(color:Pal.accent,borderRadius:BorderRadius.circular(12)),child:const Text('\u05D0\u05E9\u05E8',style:TextStyle(color:Colors.white,fontWeight:FontWeight.w900)))),
          ])],
          const SizedBox(height:20),
        ]))),
        Padding(padding:EdgeInsets.fromLTRB(24,0,24,MediaQuery.of(context).padding.bottom+16),child:Column(children:[
          if(ps.isLoading)const CircularProgressIndicator(color:Pal.premium)
          else GestureDetector(onTap:()async{await ps.loadOfferings();if(ps.packages.isNotEmpty&&mounted){final pkg=ps.packages[_plan.clamp(0,ps.packages.length-1)];final ok=await ps.purchase(pkg);if(ok&&mounted)Navigator.pop(context);}},
            child:Container(width:double.infinity,padding:const EdgeInsets.symmetric(vertical:18),
              decoration:BoxDecoration(gradient:const LinearGradient(colors:[Color(0xFFFF9F0A),Color(0xFFFF6B00)]),borderRadius:BorderRadius.circular(18),boxShadow:[BoxShadow(color:Pal.premium.withOpacity(0.5),blurRadius:16,offset:const Offset(0,4))]),
              child:Text('${_lbl[_plan]} \u2014 ${_price[_plan]}',textAlign:TextAlign.center,style:const TextStyle(color:Colors.white,fontSize:16,fontWeight:FontWeight.w900)))),
          const SizedBox(height:10),
  ...
  Widget _bf(String e,String t)=>Padding(padding:const EdgeInsets.only(bottom:12),child:Row(children:[Text(e,style:const TextStyle(fontSize:18)),const SizedBox(width:12),Expanded(child:Text(t,style:const TextStyle(color:Pal.tp,fontSize:14,fontWeight:FontWeight.w600)))]));
}
